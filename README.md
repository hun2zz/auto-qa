...
### 블랙박스 설계 기법이란? (EP / BVA / DT / ST)

**블랙박스** = 코드 속을 안 보고, **요구사항상 "입력 → 기대 출력"만 보고** 케이스를 뽑는 방식(반대는 화이트박스=코드 라인/분기 커버리지). 요구사항의 **입력 공간을 검증된 레시피로 체계적으로 쪼개** AI가 흔히 하는 "해피패스만 테스트하고 무효/경계는 빠뜨리는" 실수를 막는다. 예시는 상담 폼의 **이름(필수, 2~50자)**.

| 태그 | 기법 | 무엇을 빠짐없이 | 예시(이름 2~50자) |
|---|---|---|---|
| `[EP]` | **동등분할** | 입력을 "같은 동작 그룹(클래스)"로 나눠 **그룹당 1케이스** | 유효(30자)·빈값·공백만·1자(무효)·51자(무효) |
| `[BVA]` | **경계값** | 버그가 몰리는 **경계 바로 아래/정확히/바로 위** | 1자(거부)·**2자(통과, 하한)**·**50자(통과, 상한)**·51자(거부) |
| `[DT]` | **결정표** | 조건 2개+면 **조합마다** 결과 나열(무효 조합 누락 방지) | 이름유효×연락처유효 → 4조합, 특히 "이름 OK + 연락처 X" |
| `[ST]` | **상태전이** | 상태 바뀌는 흐름의 **정상 전이 + 잘못된 전이** | 에러표시→재입력하면 에러 사라짐 / 제출중 재클릭 |

**왜 중요:** 동등분할만 하면 "30자 통과"로 끝나 `age > 19` 같은 **off-by-one을 놓친다**. 경계값이 "2자 통과 / 1자 거부"를 콕 찍어 그런 버그를 잡는다. → 체크리스트 생성 프롬프트에 이 레시피를 명시하고, 각 항목 끝에 `[EP]`/`[BVA]`/`[DT]`/`[ST]` 태그를 붙여 **"어떤 기법으로 이 케이스가 나왔나"를 감사**할 수 있게 한다. (⚠️ 제약은 코드/요구사항에서 확인된 것만 적용 — 지어낸 경계 금지)
...
**"AI 가 없는 셀렉터를 지어내는 환각"을 막는 장치.** LLM 은 테스트를 만들 때 그럴듯한 `getByTestId('submit-btn')` 를 상상해서 쓰는데, 실제 코드엔 그런 게 없으면 테스트가 처음부터 깨진다. 그래서 **프로젝트 코드에 실제로 존재하는 셀렉터·라우트만** 뽑아 AI에게 "이 목록 안에서만 써라"고 못박는다.

```mermaid
flowchart LR
    SRC["소스 (src·app·components)"] -->|"정규식 추출"| IDX[("index.json<br/>· data-testid[]<br/>· aria-label[]<br/>· 라우트[]")]
    IDX -->|"생성 프롬프트에 주입"| GEN["🤖 체크리스트·테스트 생성<br/>'이 목록 안에서만 셀렉터 사용'"]
    IDX -->|"대조"| VAL["⚖️ 셀렉터 검증<br/>목록에 없는 getByTestId = 지어냄 🚩"]
    classDef data fill:#f5f3ff,stroke:#a78bfa,color:#4c1d95
    classDef ai fill:#eef2ff,stroke:#6366f1,color:#1e1b4b
    classDef det fill:#ecfdf5,stroke:#10b981,color:#064e3b
    class IDX data
...
- **무엇을:** 소스에서 `data-testid` / `aria-label` / 라우트(`/consultation` 등)를 정규식으로 추출 → `.qa/index/index.json` 에 저장(프로젝트 연결 시 1회 빌드, `rebuildIndex` 로 갱신).
- **두 곳에서 쓰임:** ① 생성 프롬프트에 주입 → AI가 근거 있는 셀렉터만 쓰게(환각 예방). ② **셀렉터 검증**(11번)에서 대조 → 지어낸 셀렉터 사후 적발.
- **한계:** `getByTestId` 대조가 핵심이라 testid 기반일수록 효과적(getByRole·텍스트 셀렉터는 인덱스로 못 잡음). 그래서 testid 없는 프로젝트는 인덱스가 얇고(예: grape-toy testid 0개) 환각 방지 효과가 약함.
