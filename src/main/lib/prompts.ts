...
## 네이티브 HTML 검증을 존중하라 (대량 오탐 1순위 원인) — 매우 중요
- input/textarea 에 \`required\`, \`maxLength\`, \`pattern\`, \`min\`/\`max\`, \`type="email|tel|number|url"\` 이 붙어 있으면 **브라우저가 제출을 먼저 막고**, 그러면 폼의 onSubmit(자바스크립트 커스텀 검증)이 **아예 실행되지 않는다.**
- 그래서 **빈 required 필드를 비운 채 제출하고 앱의 '커스텀 에러 문구'가 뜨길 기대하면 안 된다** — 네이티브가 막아 그 문구는 절대 안 뜬다(대표 오탐).
  · 빈값 '차단'은 이렇게 검증: \`await expect(input).toHaveJSProperty('validity.valueMissing', true)\` 또는 [제출 후 URL 미변경 / 완료화면 미표시 / 서버요청 0건] 중 하나.
  · **커스텀 에러 문구는 '비어있지 않지만 무효'인 값에서만 검증하라** (예: 1자, 공백만(\`'   '\`), 형식 위반) — 그때만 required 를 통과해 커스텀 검증 코드가 실행된다.
- \`maxLength={N}\` 이 있으면 N+1 자를 넣어도 **필드가 N 자에서 잘린다.** "N+1 자 입력 → 값이 N+1" 을 기대하지 마라. 초과는 [값이 N 자로 잘림(\`toHaveValue\` 길이 N)] 또는 [제출 차단] 으로 검증.
- 이 속성들을 **반드시 코드에서 Read 로 먼저 확인**하고 그에 맞춰 단언 방식을 고른다.
## 테스트 데이터 격리 (strict-mode·누적 오염 방지)
- 테스트가 실제 API로 데이터를 만들면(폼 제출 → DB 저장 등) 같은 고정값(이름·전화)을 재사용할 경우 **실행마다 행이 누적**돼 나중엔 셀렉터가 여러 행에 매칭된다(strict-mode 위반).
- 생성 데이터에는 **테스트마다 고유 값**을 써라(예: 전화 뒷자리·이름에 짧은 유니크 토큰). 목록에서 그 행을 찾을 때도 그 고유 값으로 스코프해 정확히 1행만 잡는다.
## API 상태코드는 과다특정 금지
- 코드에서 정확한 코드를 확인 못 했으면 **범위로** 단언하라(무효 입력 → 4xx, 성공 → 2xx).
- **rate limit(429)** 가 있는 엔드포인트는 연속 요청 시 429 가 '정상' 응답일 수 있다 → 201 만 기대하지 말고 429 를 허용하거나(\`expect([201,429]).toContain(status)\`) 요청 간 고유성/간격으로 회피.
- route 가로챌 때 한 요청에 \`fulfill\` 과 \`continue\` 를 **동시에 호출하지 마라** ('Route is already handled!'). 분기해서 정확히 한 번만 처리한다.`
